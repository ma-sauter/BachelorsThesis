...
def draw_dot_and_arrow(dotx, arrowcolor="black"):
    ...
    plt.plot(
        ...
        "+",
        color=arrowcolor,
        markersize=plotfactor * 10,
        markeredgewidth=plotfactor * 3,
    )
    plt.plot(
        x_list1, tangent(x_list1, dotx), "--", color="gray", linewidth=plotfactor * 0.7
    )
    ...
    ax.annotate(
        ...
        xytext=(dotx, f(dotx)),
        xy=(
            dotx + learning_rate * (-df(dotx)),
            tangent(dotx + learning_rate * (-df(dotx)), dotx),
        ),
        arrowprops=dict(
            lw=plotfactor * 1.5,
            shrinkB=0,
    ...
    plt.plot(
        [dotx + learning_rate * (-df(dotx)), dotx + learning_rate * (-df(dotx))],
        [
            tangent(dotx + learning_rate * (-df(dotx)), dotx),
            f(dotx + learning_rate * (-df(dotx))),
        ],
        "-",
        color=arrowcolor,
    )
...
fig, ax = plt.subplots(figsize=(390 / 72, 390 / 4 / 72))
